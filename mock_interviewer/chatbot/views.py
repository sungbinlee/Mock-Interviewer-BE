...
class ChatGPTAPI(APIView):
    def get(self, request):
        user = request.user
        today = date.today()
        chat_request_count = UserChatRequest.objects.filter(
        ...
        conversations = [
            {"role": chat.role, "content": chat.content} for chat in user_chats
        ]

        return Response(
            {
                "conversations": conversations,
                "count": chat_request_count.request_count,
                "limit": limit,
            },
            status=status.HTTP_200_OK,
        )
    ...
    def post(self, request):
        ...
        UserChatRequest.objects.filter(pk=user_chat_request.pk).update(
        ...
        if not user_input and not chats:
            ...
            ai_chat = Chat(
                user=request.user,
                role=system_start_message["role"],
                content=system_start_message["content"],
            )
            ai_chat.save()
        ...
        # 응답값 프론트엔드로 전달
        return Response(
            {"response": ai_response, "audio_url": audio_url}, status=status.HTTP_200_OK
        )
